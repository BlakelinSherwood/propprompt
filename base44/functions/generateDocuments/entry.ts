...
async function renderClientPortfolioPdf(doc, data, branding) {
  ...
  if (data.tiered_comps?.tiers) {
    ...
    if (topTier?.comps?.length) {
      ...
      drawTable(doc, margin, y, ['Address', 'Date', 'Price', 'Adj $/SF'], rows, [200, 70, 80, contentWidth - 358], { headerFill: branding.primary_color || '#1A3226', headerTextColor: '#FFFFFF', fontSize: 8, rowHeight: 22, branding });
    }
  }
