...
const ROLE_LABELS = {
  brokerage_admin: "Brokerage Admin", team_lead: "Team Lead", agent: "Agent",
  assistant: "Assistant", team_agent: "Team Agent",
};
const STATUS_COLORS = {
...
export default function BrokerageMembersTab({ org, user }) {
  ...
  useEffect(() => {
    base44.entities.OrgMembership.filter({ org_id: org.id }).then((data) => {
      setMembers(data);
      setLoading(false);
    });
  }, [org.id]);
  ...
  async function handleInvite() {
    ...
    setMembers((prev) => [...prev, { user_email: inviteEmail, org_id: org.id, role_in_org: inviteRole, status: "pending_invite" }]);
    setInviteEmail("");
    setInviting(false);
    toast({ title: "Invite created", description: `${inviteEmail} has been invited as ${ROLE_LABELS[inviteRole]}.` });
  }

  async function toggleStatus(member) {
    const newStatus = member.status === "active" ? "suspended" : "active";
    await base44.entities.OrgMembership.update(member.id, { status: newStatus });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative max-w-xs">
        ...
        <Input className="pl-9 h-9 text-sm" placeholder="Search members…" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      ...
      <div className="rounded-xl border border-[#1A3226]/10 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[#1A3226]/5">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-12 text-center text-[#1A3226]/40">No members found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
