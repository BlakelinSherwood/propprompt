...
const STATUS_COLORS = {
  ...
  draft: "bg-gray-100 text-gray-600", failed: "bg-red-100 text-red-700", archived: "bg-gray-100 text-gray-400",
};

const ASSESSMENT_LABELS = {
  listing_pricing: "Listing Pricing", buyer_intelligence: "Buyer Intelligence",
  investment_analysis: "Investment", cma: "CMA", rental_analysis: "Rental",
};

export default function TeamAnalysesTab({ org, user }) {
  ...
  useEffect(() => {
    base44.entities.Analysis.filter({ org_id: org.id }, "-created_date", 100).then((d) => { setAnalyses(d); setLoading(false); });
  }, [org.id]);

  const filtered = analyses.filter((a) =>
    (a.intake_data?.address || "").toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        {!isPlatformOwner && <span className="text-xs flex items-center gap-1 text-[#1A3226]/40"><Lock className="w-3 h-3" /> Private analyses redacted</span>}
      </div>
      ...
      <div className="rounded-xl border border-[#1A3226]/10 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[#1A3226]/5">
              ...
              <tr key={a.id} className="hover:bg-[#1A3226]/[0.02]">
                ...
                <td className="px-4 py-3 text-right">
                  {!a.is_private || isPlatformOwner ? (
                    <button className="text-[#1A3226]/40 hover:text-[#1A3226]" onClick={() => navigate(`/Analysis/${a.id}`)}>
            ...
            {filtered.length === 0 && <tr><td colSpan={6} className="px-4 py-12 text-center text-[#1A3226]/40">No analyses found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
